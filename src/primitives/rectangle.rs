...
fn rect_pixels(rect: &Rectangle, filled: bool) -> Vec<Pixel> {
    ...
    if ! filled {
        ...
        for (cur, next) in corners.iter().zip(corners.iter().skip(1).chain(corners.iter().take(1))) {
            pixels.extend(Line{from: *cur, to: *next}.pixels());
        }
    } else {
        // Horizontal scan-lines from top left to top right offsetting on each iteration
        for y_offset in 0..rect.height {
            pixels.extend(Line{from: corners[0] + (0, y_offset), to: corners[1] + (0, y_offset)}.pixels());
        }
    }
