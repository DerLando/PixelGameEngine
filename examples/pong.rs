...
use pixel_game_engine::{
    buffer::Buffer,
    color::DefaultColors,
    engine,
    events::{EventLoop, KeyEvent},
    pixel::Pixel,
};
...
fn clamp_player_velocity(velocity: (f32, f32)) -> (f32, f32) {
    if velocity.1 >= MAX_PLAYER_VELOCITY {
        (0.0, MAX_PLAYER_VELOCITY)
    } else {
        velocity
    }
}
...
fn left_player_collides(player: &Player, ball_position: &Pixel) -> bool {
    ...
    if ball_position.x() - RADIUS >= player.position.x() + PLAYER_WIDTH {
        false
    } else {
        if (ball_position.y() - RADIUS > player.position.y() + PLAYER_HEIGHT)
            | (ball_position.y() + RADIUS < player.position.y())
        {
            false
        } else {
            true
...
fn right_player_collides(player: &Player, ball_position: &Pixel) -> bool {
    // early exit, if we are not at the player line
    if ball_position.x() + RADIUS <= player.position.x() {
        false
    } else {
        if (ball_position.y() - RADIUS > player.position.y() + PLAYER_HEIGHT)
            | (ball_position.y() + RADIUS < player.position.y())
...
fn is_point_win(ball_position: &Pixel) -> Option<PointWin> {
    if ball_position.x() - RADIUS <= PLAYER_OFFSET {
        return Some(PointWin::Left);
    }
    if ball_position.x() + RADIUS >= WIDTH - PLAYER_OFFSET {
        return Some(PointWin::Right);
    }
...
fn is_player_at_boundary(player: &Player) -> bool {
    if player.position.y() + PLAYER_HEIGHT >= HEIGHT {
        true
    } else {
        false
    }
...
fn main() -> ! {
    ...
    let update = |s: &mut State| {
        ...
        s.player_left.position = (
            s.player_left.position.x(),
            (s.player_left.position.y() as f32 + s.player_left.velocity.1) as u32,
        )
            .into();

        s.player_right.velocity = clamp_player_velocity(s.player_right.velocity);
        s.player_right.position = (
            s.player_right.position.x(),
            (s.player_right.position.y() as f32 + s.player_right.velocity.1) as u32,
        )
            .into();
        ...
        if is_player_at_boundary(&s.player_left) {
            s.player_left.velocity = (0.0, 0.0)
        }
        if is_player_at_boundary(&s.player_right) {
            s.player_right.velocity = (0.0, 0.0)
        }
    };
    ...
    // key handlers
    let key_movement = |s: &mut State, e: &KeyEvent| match e {
    ...
    // create engine
    let builder = PixelGameEngineBuilder::new(state)
        .with_width(WIDTH)
        .with_height(HEIGHT)
        .with_update(update)
        .with_draw(draw)
